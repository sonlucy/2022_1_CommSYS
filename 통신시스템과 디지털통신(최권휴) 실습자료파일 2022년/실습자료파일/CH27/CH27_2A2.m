...
for snr_i=1:length(EbN0dBvector)
    ...
    while errcnt<100
        ...
        %%%Multipath fading channel and the received signal generation
        ht=ht_mp_ch(max_delay,L,decay_base,t_step);
        OFDM_frame_after_ht=conv(OFDM_frame,ht);
        frame_length=length(OFDM_frame_after_ht);
        noise=sqrt(vn)*(randn(1,frame_length)+j*randn(1,frame_length));
